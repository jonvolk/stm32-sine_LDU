...
static s32fp ProcessUdc()
{
...
#if CONTROL == CTRL_SINE
   {
      ...
      //s32fp fweak = Param::Get(Param::fweak); original code
      ...
      {
         fweakVal = MAP(Param::GetInt(Param::potnom), 36, 100, (Param::GetInt(Param::fweakstrt)), (Param::GetInt(Param::fweak)));
      }
      else
      {
         fweakVal = Param::GetInt(Param::fweakstrt);
      } 
...
static s32fp ProcessThrottle()
{
   ...
   if ((int)Encoder::GetSpeed() < Param::GetInt(Param::throtramprpm))
      ...
      Throttle::throttleRamp = modThrotRamp; //Param::Get(Param::throtramp); TRASH 
   else
